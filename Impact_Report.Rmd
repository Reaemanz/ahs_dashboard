...
# Add index to original data
original_data <- Mapping_Kanungu_2019_Year5_to_Kanungu_Year6_Control %>%
  mutate(RowID = row_number()) %>% view()

# Pivot long, but drop the original columns from the pivoted frame
pivoted_data <- original_data %>%
  pivot_longer(
    cols = c(RTV, Control),
    names_to = "Status",
    values_to = "Values",
  ...
  select(RowID, Status, Values) %>% view() 

# Join back without duplicating columns
Income_streams <- pivoted_data %>%
  left_join(original_data, by = "RowID") %>%
  select(
    District,
    `Income Streams`,
    Cohort,
...
write.csv(Income_streams, "Income_streams.csv", row.names = FALSE)
write.csv(pivoted_data_2, "Income_streams_2.csv", row.names = FALSE)
write.csv(Mapping_Kanungu_2019_Year5_to_Kanungu_Year6_Control, "original_income_streams.csv", row.names = FALSE)
