...
combined_RTV_Control_Graduated_Clusters_pivoted <- combined_RTV_Control_Graduated_Clusters %>%
  mutate(Amount = as.numeric(Amount),
         `Income Streams` = str_squish(`Income Streams`)) %>%              # Ensure Amount is numeric
  pivot_wider(
    names_from = Status,
    values_from = Amount
  ) %>% 
...
Mapping_2018_RTV_Control_Graduated_Clusters_final <- combined_RTV_Control_Graduated_Clusters_pivoted %>%
...
# Get reference Control values from Kanungu, Cohort 2018, Year "Year 1"
kanungu_Controls_2019 <- Mapping_2018_RTV_Control_Graduated_Clusters_final %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "12 months") %>% view()
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2018 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_2018_RTV_Control_Graduated_Clusters_final <- Mapping_2018_RTV_Control_Graduated_Clusters_final %>%
  left_join(kanungu_reference, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2019 &
      Year == "Base" &
      District %in% c("Kisoro", "Rukunjiri"),
      Control_Kanungu_2018,
      Control
    )
  ) %>%
  select(-Control_Kanungu_2018)
