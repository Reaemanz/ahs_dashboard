...
2020 COHORT CONTROL- YEAR 1 [KISORO, KANUNGU, RUKUNGIRI]
...
Mitooma_filtered <- Mapping_Mitooma_Year1_2020_Controls_to_Mitooma_Yr1 %>%
  filter(!(District == "Mitooma" & Cohort == "2020" & Year == "Year 1" & is.na(RTV))) %>% view()
...
```{r}
Mitooma_Controls_2020_Year1 <- Mitooma_filtered %>%
  filter(District == "Mitooma", Cohort == "2020", Year == "Year 1", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Mitooma_2020 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Kagadi_Base_2020_Controls_to_Mitooma_Yr1 <- Mitooma_filtered %>%
  left_join(Mitooma_Controls_2020_Year1, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
2022 COHORT CONTROL-YEAR [Mitooma Yr 2]
# Mitooma
...
2022 COHORT CONTROL- YEAR 1 [Kibaale Base]
# Kagadi Non Gac
...
2018 COHORT - CONTROL YEAR TWO


```{r}

# Extract the Control values from Kanungu
kanungu_controls_Year2 <- Mapping_Kagadi_NonGac_2022_Year1_to_Kibaale_Control %>%
  filter(District == "Kanungu", Cohort == 2018, Year == "Year 2", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2018 = Control) %>% view()

# Update only the NA Control values in Kanungu
Mapping_KaNUNGU_2022_Year2_to_Kanungu_Control <- Mapping_Kagadi_NonGac_2022_Year1_to_Kibaale_Control %>%
  left_join(kanungu_controls_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
    is.na(Control) & District == "Kanungu" & Cohort == 2018 & Year == "Year 2",
    Control_Kanungu_2018,
    Control
  )) %>%
  select(-Control_Kanungu_2018) %>% view()
...
2019 COHORT - CONTROL Year 2
# Kanungu


```{r}

kanungu_Controls_2019_Year2 <- Mapping_KaNUNGU_2022_Year2_to_Kanungu_Control %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 3", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2019 = Control) %>% view()

# Fill NAs in Control for Kanungu, Cohort 2019, Year "Base"
Mapping_Kanungu_2019_to_Kan_Yr2_Controls <- Mapping_KaNUNGU_2022_Year2_to_Kanungu_Control %>%
  left_join(kanungu_Controls_2019_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2019 &
...
2019 COHORT - CONTROL YEAR TWO
# Rukungiri

```{r}

kanungu_Controls_2019_Year2 <- Mapping_Kanungu_2019_to_Kan_Yr2_Controls %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 3", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2019 = Control) %>% view()

# Fill NAs in Control for Kanungu, Cohort 2019, Year "Base"
Mapping_2019_to_Rukungiri_Yr2_Controls <- Mapping_Kanungu_2019_to_Kan_Yr2_Controls %>%
  left_join(kanungu_Controls_2019_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
2020 COHORT CONTROL- YEAR 2 [KANUNGU]
# [KISORO, KANUNGU, RUKUNGIRI]

```{r}


kanungu_Controls_2020_Year2 <- Mapping_2019_to_Rukungiri_Yr2_Controls %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 4", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2020 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_2020_Year1_to_Kan_Yr2_Controls <- Mapping_2019_to_Rukungiri_Yr2_Controls %>%
  left_join(kanungu_Controls_2020_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2020 &
      Year == "Year 2" &
      District %in% c("Rukungiri", "Kanungu"),
      Control_Kanungu_2020,
      Control
    )
  ) %>%
...
2020 COHORT CONTROL- YEAR TWO
# Mitooma

```{r}

Mitooma_Controls_2020_Year2 <- Mapping_2020_Year1_to_Kan_Yr2_Controls %>%
  filter(District == "Mitooma", Cohort == "2020", Year == "Year 2", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Mitooma_2020 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Mitooma_Year2_2020_to_Mitooma_Yr2_Controls <- Mapping_2020_Year1_to_Kan_Yr2_Controls %>%
  left_join(Mitooma_Controls_2020_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2020 &
...
Mitooma_filtered_Year2 <- Mapping_Mitooma_Year2_2020_to_Mitooma_Yr2_Controls %>%
  filter(!(District == "Mitooma" & Cohort == "2020" & Year == "Year 2" & is.na(RTV))) %>% view()

```

2020 COHORT CONTROL- Year 2
# KAGADI

```{r}
Mitooma_Controls_2020_Year2 <- Mitooma_filtered_Year2 %>%
  filter(District == "Mitooma", Cohort == "2020", Year == "Year 2", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Mitooma_2020 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Kagadi_Base_2020_to_Mitooma_Yr2_Controls <- Mitooma_filtered_Year2 %>%
  left_join(Mitooma_Controls_2020_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
2021 COHORT CONTROL- YEAR TWO [KANUNGU]
# KANUNGU

```{r}


Kanungu_Controls_2021_Year2 <- Mapping_Kagadi_Base_2020_to_Mitooma_Yr2_Controls %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 5", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2021 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Kanungu_2021_Year2_to_kanungu_Yr5_Controls <- Mapping_Kagadi_Base_2020_to_Mitooma_Yr2_Controls %>%
  left_join(Kanungu_Controls_2021_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2021 &
...
```{r}
Kanungu_Controls_2021_Year2 <- Mapping_Kanungu_2021_Year2_to_kanungu_Yr5_Controls %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 5", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2021 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Rukunjiri_2021_Year2_to_kanungu_Yr3_Controls <- Mapping_Kanungu_2021_Year2_to_kanungu_Yr5_Controls %>%
  left_join(Kanungu_Controls_2021_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
Mitooma_Controls_2021_Year2 <- Mapping_Rukunjiri_2021_Year2_to_kanungu_Yr3_Controls %>%
  filter(District == "Mitooma", Cohort == "2020", Year == "Year 3", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Mitooma_2021 = Control) %>% view()

# Fill NAs in Control for Mitooma, Cohort 2021, Year "Base"
Mapping_Mitooma_2021_Year2_to_Mitooma__Controls_Yr3 <- Mapping_Rukunjiri_2021_Year2_to_kanungu_Yr3_Controls %>%
  left_join(Mitooma_Controls_2021_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2021 &
...
Rubanda_Controls_2022_Year2 <- Mapping_Mitooma_2021_Year2_to_Mitooma__Controls_Yr3 %>%
  filter(District == "Rubanda", Cohort == "2021", Year == "Year 2", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Rubanda_2022 = Control) %>% view()

# Fill NAs in Control for Mitooma, Cohort 2021, Year "Base"
Mapping_Rubanda_2022_Year2_to_Rubanda_Yr2_Control <- Mapping_Mitooma_2021_Year2_to_Mitooma__Controls_Yr3 %>%
  left_join(Rubanda_Controls_2022_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2021 &
...
2022 COHORT CONTROL- YEAR TWO [KANUNGU]
# KANUNGU

```{r}


Kanungu_Controls_2022_Year2 <- Mapping_Rubanda_2022_Year2_to_Rubanda_Yr2_Control %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 6", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2021 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Kanungu_2022_Year2_to_kanungu_Yr6_Controls <- Mapping_Rubanda_2022_Year2_to_Rubanda_Yr2_Control %>%
  left_join(Kanungu_Controls_2022_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2022 &
...
2022 COHORT CONTROL- YEAR2 [Kanungu Yr 6]
# Rukungiri

```{r}
Kanungu_Controls_2022_Year2 <- Mapping_Kanungu_2022_Year2_to_kanungu_Yr6_Controls %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 6", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2022 = Control) %>% view()

# Fill NAs in Control for Mitooma, Cohort 2021, Year "Base"
Mapping_Rukungiri_2022_Year2_to_Kanungu_Yr5_Control <- Mapping_Kanungu_2022_Year2_to_kanungu_Yr6_Controls %>%
  left_join(Kanungu_Controls_2022_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
```{r}
Mitooma_Controls_2022_Year2 <- Mapping_Rukungiri_2022_Year2_to_Kanungu_Yr5_Control %>%
  filter(District == "Mitooma", Cohort == "2020", Year == "Year 4", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Mitooma_2022 = Control) %>% view()

# Fill NAs in Control for Mitooma, Cohort 2021, Year "Base"
Mapping_Mitooma_2022_Year2_to_Mitooma_Yr4_Control <- Mapping_Rukungiri_2022_Year2_to_Kanungu_Yr5_Control %>%
  left_join(Mitooma_Controls_2022_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2022 &
...
2022 COHORT CONTROL- YEAR 2 [Mitooma Yr 2]
# Rubirizi

```{r}

Mitooma_Controls_2022_Year2 <- Mapping_Mitooma_2022_Year2_to_Mitooma_Yr4_Control %>%
  filter(District == "Mitooma", Cohort == "2020", Year == "Year 4", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Mitooma_2022 = Control) %>% view()

# Fill NAs in Control for Mitooma, Cohort 2021, Year "Base"
Mapping_Rubirizi_2022_Year2_to_Mitooma_Yr2_Control <- Mapping_Mitooma_2022_Year2_to_Mitooma_Yr4_Control %>%
  left_join(Mitooma_Controls_2022_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
Rubanda_Controls_2022_Year2 <- Mapping_Rubirizi_2022_Year2_to_Mitooma_Yr2_Control %>%
  filter(District == "Rubanda", Cohort == "2021", Year == "Year 3", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Rubanda_2022 = Control) %>% view()

# Fill NAs in Control for Mitooma, Cohort 2021, Year "Base"
Mapping_Rubanda_2022_Year2_to_Rubanda_Yr2_Control <- Mapping_Rubirizi_2022_Year2_to_Mitooma_Yr2_Control %>%
  left_join(Rubanda_Controls_2022_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
2022 COHORT CONTROL- YEAR 2 [Kyenjojo Year 2]
# Kyenjojo

```{r}
Kibaale_Controls_2022_Year2 <- Mapping_Rubanda_2022_Year2_to_Rubanda_Yr2_Control %>%
  filter(District == "Kibaale", Cohort == "2022", Year == "Year 2", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kibaale_2022 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kyenjojo_2022_Year2_to_Kibaale_Year_Control <- Mapping_Rubanda_2022_Year2_to_Rubanda_Yr2_Control %>%
  left_join(Kibaale_Controls_2022_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2022 &
...
Kibaale_Controls_2022_Year2 <- Mapping_Kyenjojo_2022_Year2_to_Kibaale_Year_Control %>%
  filter(District == "Kibaale", Cohort == "2022", Year == "Year 2", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kibaale_2022 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kagadi_Gac_2022_Year2_to_Kibaale_Year1_Control <- Mapping_Kyenjojo_2022_Year2_to_Kibaale_Year_Control %>%
  left_join(Kibaale_Controls_2022_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
Kibaale_Controls_2022_Year2 <- Mapping_Kagadi_Gac_2022_Year2_to_Kibaale_Year1_Control %>%
  filter(District == "Kibaale", Cohort == "2022", Year == "Year 2", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kibaale_2022 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kagadi_NonGac_2022_Year2_to_Kibaale_Control <- Mapping_Kagadi_Gac_2022_Year2_to_Kibaale_Year1_Control %>%
  left_join(Kibaale_Controls_2022_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
2022 COHORT CONTROL-YEAR 2 [Kaliro Base]
# Kaliro


```{r}
Kaliro_Controls_2022_Year2 <- Mapping_Kagadi_NonGac_2022_Year2_to_Kibaale_Control %>%
  filter(District == "Kaliro", Cohort == "2022", Year == "Year 2", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kaliro_2022 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kaliro_2022_Year2_to_Kaliro_Year1_Control <- Mapping_Kagadi_NonGac_2022_Year2_to_Kibaale_Control %>%
  left_join(Kaliro_Controls_2022_Year2, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2022 &
...
# Filter out Kaliro 2022 Year 1 copy
Kaliro_filtered_Year2 <- Mapping_Kaliro_2022_Year2_to_Kaliro_Year1_Control %>%
  filter(!(District == "Kaliro" & Cohort == "2022" & Year == "Year 2" & is.na(RTV))) %>% view()

```

# FILTER YEAR 1 AND YEAR 2


```{r}

# Filter out Kanungu 2018 Year 1 
Kanungu_2018_filtered_Year1 <- Kaliro_filtered_Year2 %>%
  filter(!(District == "Kanungu" & Cohort == "2018" & Year == "Year 1" & is.na(RTV))) %>% view()

# Filter out Rubanda 2018 Year 1 
Rubanda_2018_filtered_Year1 <- Kanungu_2018_filtered_Year1 %>%
  filter(!(District == "Rubanda" & Cohort == "2018" & Year == "Year 1" & is.na(RTV))) %>% view()

# Filter out Rubanda 2021 Year 1 
Rubanda_2021_filtered_Year1 <- Rubanda_2018_filtered_Year1 %>%
  filter(!(District == "Rubanda" & Cohort == "2021" & Year == "Year 1" & is.na(RTV))) %>% view()

# Filter out Kibaale 2018 Year 1 
Kibaale_2022_filtered_Year2 <- Rubanda_2021_filtered_Year1 %>%
  filter(!(District == "Kibaale" & Cohort == "2021" & Year == "Year 1" & is.na(RTV))) %>% view()

# Filter out Kibaale 2022 Year 1 
Kibaale_2022_filtered_Year1 <- Kibaale_2022_filtered_Year2 %>%
  filter(!(District == "Kibaale" & Cohort == "2022" & Year == "Year 1" & is.na(RTV))) %>% view()

# Filter out Kanungu 2018 Year 2 
Kanungu_2018_filtered_Year2 <- Kibaale_2022_filtered_Year1 %>%
  filter(!(District == "Kanungu" & Cohort == "2018" & Year == "Year 2" & is.na(RTV))) %>% view()


# Filter out Rubanda 2018 Year 2
Rubanda_2021_filtered_Year2 <- Kanungu_2018_filtered_Year2 %>%
  filter(!(District == "Rubanda" & Cohort == "2021" & Year == "Year 2" & is.na(RTV))) %>% view()

# Filter out Kaliro 2018 Year 2
Kaliro_2021_filtered_Year2 <- Rubanda_2021_filtered_Year2 %>%
  filter(!(District == "Kaliro" & Cohort == "2022" & Year == "Year 2" & is.na(RTV))) %>% view()

# Filter out Kibaale 2022 Year 2
Kibaale_2022_filtered_Year2 <- Kaliro_2021_filtered_Year2 %>%
  filter(!(District == "Kibaale" & Cohort == "2022" & Year == "Year 2" & is.na(RTV))) %>% view()

# Filter out Kibaale 2022 Base
Kibaale_2022_filtered_Base <- Kibaale_2022_filtered_Year2 %>%
  filter(!(District == "Kibaale" & Cohort == "2022" & Year == "Base" & is.na(RTV))) %>% view()

# Filter out Kanungu 2018 Year 6
Kanungu_2018_filtered_Year_6 <- Kibaale_2022_filtered_Base %>%
  filter(!(District == "Kanungu" & Cohort == "2018" & Year == "Year 6" & is.na(RTV)& is.na(Control))) %>% view()
...
Kanungu_Year_3_Control_18 <- Kanungu_2018_filtered_Year_6 %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 3", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2018 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kanungu_2018_Year3_to_Kanungu_Year3_Control <- Kanungu_2018_filtered_Year_6 %>%
  left_join(Kanungu_Year_3_Control_18, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2018 &
...
# Filter out Kanungu 2018 Year 3
Kanungu_2018_filtered_Year_3 <- Mapping_Kanungu_2018_Year3_to_Kanungu_Year3_Control %>%
  filter(!(District == "Kanungu" & Cohort == "2018" & Year == "Year 3" & is.na(RTV))) %>% view()
...
Kanungu_Year_3_Control_18 <- Kanungu_2018_filtered_Year_3 %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 4", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2019 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kanungu_2019_Year3_to_Kanungu_Year4_Control <- Kanungu_2018_filtered_Year_3 %>%
  left_join(Kanungu_Year_3_Control_18, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
# Filter out Kanungu 2018 Year 3
Kanungu_2019_filtered_Year_3 <- Mapping_Kanungu_2019_Year3_to_Kanungu_Year4_Control %>%
  filter(!(District == "Kanungu" & Cohort == "2018" & Year == "Year 4" & is.na(RTV))) %>% view()

```

2019 COHORT,KANUNGU CONTROL- YEAR THREE [KANUNGU]
# RUKUNJIRI
...
# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Rukunjiri_2019_Year3_to_kanungu_Yr4_Controls <- Kanungu_2019_filtered_Year_3 %>%
  left_join(Kanungu_Year_3_Control_18, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
Kanungu_Year_3_Control_20 <- Mapping_Rukunjiri_2019_Year3_to_kanungu_Yr4_Controls %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 5", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2020 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kanungu_2020_Year3_to_Kanungu_2018_Year5_Control <- Mapping_Rukunjiri_2019_Year3_to_kanungu_Yr4_Controls %>%
  left_join(Kanungu_Year_3_Control_20, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2020 &
...
# Filter out Kanungu 2018 Year 3
Kanungu_2020_filtered_Year_3 <- Mapping_Kanungu_2020_Year3_to_Kanungu_2018_Year5_Control %>%
  filter(!(District == "Kanungu" & Cohort == "2018" & Year == "Year 5" & is.na(RTV))) %>% view()

```


2020 COHORT,KANUNGU CONTROL- YEAR THREE [KANUNGU]
# RUKUNJIRI

```{r}
Rukunjiri_Year_3_Control_20 <- Mapping_Rukunjiri_2019_Year3_to_kanungu_Yr4_Controls %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 5", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Rukunjiri_2020 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Rukunjiri_2020_Year3_to_kanungu_Yr4_Controls <- Kanungu_2020_filtered_Year_3 %>%
  left_join(Rukunjiri_Year_3_Control_20, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2020 &
      Year == "Year 3" &
      District %in% c("Rukungiri"),
      Control_Rukunjiri_2020,
      Control
    )
  ) %>%
  select(-Control_Rukunjiri_2020) %>% view()

```


2020 COHORT,KANUNGU CONTROL- YEAR THREE [MITOOMA]
# KAGADI

```{r}
Mitooma_Year_3_Control_20 <- Mapping_Rukunjiri_2020_Year3_to_kanungu_Yr4_Controls %>%
  filter(District == "Mitooma", Cohort == "2020", Year == "Year 3", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Mitooma_2020 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Kagadi_2020_Year3_to_Mitooma_Yr3_Controls <- Mapping_Rukunjiri_2020_Year3_to_kanungu_Yr4_Controls %>%
  left_join(Mitooma_Year_3_Control_20, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2020 &
...
Mitooma_Year_3_Control <- Mapping_Kagadi_2020_Year3_to_Mitooma_Yr3_Controls %>%
  filter(District == "Mitooma", Cohort == "2020", Year == "Year 3", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Mitooma_2020 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Mitooma_2020_Year3_to_Mitooma_Year3_Control <- Mapping_Kagadi_2020_Year3_to_Mitooma_Yr3_Controls %>%
  left_join(Mitooma_Year_3_Control, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2020 &
...
# Filter out Kanungu 2018 Year 3
Mitooma_2020_filtered_Year_3 <- Mapping_Mitooma_2020_Year3_to_Mitooma_Year3_Control %>%
  filter(!(District == "Mitooma" & Cohort == "2020" & Year == "Year 3" & is.na(RTV))) %>%
  view()
...
Kanungu_Year_3_Control_21 <- Mitooma_2020_filtered_Year_3 %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 6", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2021 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kanungu_2021_Year3_to_Kanungu_2018_Year6_Control <- Mitooma_2020_filtered_Year_3 %>%
  left_join(Kanungu_Year_3_Control_21, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2021 &
...
# Filter out Kanungu 2018 Year 3
Kanungu_2021_filtered_Year_3 <- Mapping_Kanungu_2021_Year3_to_Kanungu_2018_Year6_Control %>%
  filter(!(District == "Kanungu" & Cohort == "2018" & Year == "Year 6" & is.na(RTV))) %>% view()
...
Rubanda_Year_3_Control <- Mapping_Kanungu_2021_Year3_to_Kanungu_2018_Year6_Control %>%
  filter(District == "Rubanda", Cohort == "2021", Year == "Year 3", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Rubanda_Control_2020 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Rubanda_2021_Year3_to_Rubanda_Year3_Control <- Mapping_Kanungu_2021_Year3_to_Kanungu_2018_Year6_Control %>%
  left_join(Rubanda_Year_3_Control, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2021 &
      Year == "Year 3" &
      District %in% c("Rubanda"),
      Rubanda_Control_2020,
      Control
    )
  ) %>%
  select(-Rubanda_Control_2020) %>% view()

# Filter out Kanungu 2018 Year 3
Rubanda_2021_filtered_Year_3 <- Mapping_Rubanda_2021_Year3_to_Rubanda_Year3_Control %>%
  filter(!(District == "Rubanda" & Cohort == "2021" & Year == "Year 3" & is.na(RTV))) %>%
  view()

```


2021 COHORT,KANUNGU CONTROL- YEAR THREE [KANUNGU]
# RUKUNJIRI

```{r}
Rukunjiri_Year_3_Control_21 <- Rubanda_2021_filtered_Year_3 %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 6", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Rukunjiri_2021 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Rukunjiri_2021_Year3_to_kanungu_Yr4_Controls <- Rubanda_2021_filtered_Year_3 %>%
  left_join(Rukunjiri_Year_3_Control_21, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2021 &
      Year == "Year 3" &
      District %in% c("Rukungiri"),
      Control_Rukunjiri_2021,
      Control
    )
  ) %>%
  select(-Control_Rukunjiri_2021) %>% view()
...
Mitooma_Year_3_Control_21 <- Mapping_Rukunjiri_2021_Year3_to_kanungu_Yr4_Controls %>%
  filter(District == "Mitooma", Cohort == "2020", Year == "Year 4", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Mitooma_2021 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Mitooma_2021_Year3_to_Mitooma_Year3_Control <- Mapping_Rukunjiri_2021_Year3_to_kanungu_Yr4_Controls %>%
  left_join(Mitooma_Year_3_Control_21, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2021 &
...
# Filter out Kanungu 2018 Year 3
Mitooma_2021_filtered_Year_3 <- Mapping_Mitooma_2021_Year3_to_Mitooma_Year3_Control %>%
  filter(!(District == "Mitooma" & Cohort == "2020" & Year == "Year 4" & is.na(RTV))) %>%
  view()
...
Kanungu_Year_4_Control_18 <- Kanungu_2018_filtered_Year_3 %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 4", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2018 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kanungu_2018_Year4_to_Kanungu_Year4_Control <- Mitooma_2021_filtered_Year_3 %>%
  left_join(Kanungu_Year_4_Control_18, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2018 &
...
Kanungu_Year_5_Control_19 <- Mapping_Rukunjiri_2019_Year3_to_kanungu_Yr4_Controls %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 5", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2019 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kanungu_2019_Year4_to_Kanungu_Year5_Control <- Mapping_Kanungu_2018_Year4_to_Kanungu_Year4_Control %>%
  left_join(Kanungu_Year_5_Control_19, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2019 &
...
2019 COHORT,KANUNGU CONTROL- YEAR FOUR [KANUNGU]
# RUKUNJIRI

```{r}
Rukunjiri_Year_4_Control_18 <- Kanungu_2018_filtered_Year_3 %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 5", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Rukunjiri_2019 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Rukunjiri_2019_Year4_to_kanungu_Yr5_Controls <- Mapping_Kanungu_2019_Year4_to_Kanungu_Year5_Control %>%
  left_join(Rukunjiri_Year_4_Control_18, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2019 &
      Year == "Year 4" &
      District %in% c("Rukungiri"),
      Control_Rukunjiri_2019,
      Control
    )
  ) %>%
  select(-Control_Rukunjiri_2019) %>% view()
...
Kanungu_Year_4_Control_20 <- Mapping_Rukunjiri_2019_Year3_to_kanungu_Yr4_Controls %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 6", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2020 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kanungu_2020_Year4_to_Kanungu_2018_Year6_Control <- Mapping_Rukunjiri_2019_Year4_to_kanungu_Yr5_Controls %>%
  left_join(Kanungu_Year_4_Control_20, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2020 &
...
2020 COHORT,KANUNGU CONTROL- YEAR FOUR [KANUNGU]
# RUKUNJIRI

```{r}
Rukunjiri_Year_4_Control_20 <- Mapping_Rukunjiri_2019_Year3_to_kanungu_Yr4_Controls %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 6", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Rukunjiri_2020 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Rukunjiri_2020_Year4_to_kanungu_Yr4_Controls <- Mapping_Kanungu_2020_Year4_to_Kanungu_2018_Year6_Control %>%
  left_join(Rukunjiri_Year_4_Control_20, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2020 &
...
Mitooma_Year_4_Control <- Mapping_Kagadi_2020_Year3_to_Mitooma_Yr3_Controls %>%
  filter(District == "Mitooma", Cohort == "2020", Year == "Year 4", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Mitooma_2020 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Mitooma_2020_Year4_to_Mitooma_Year4_Control <- Mapping_Rukunjiri_2020_Year4_to_kanungu_Yr4_Controls %>%
  left_join(Mitooma_Year_4_Control, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2020 &
...
2020 COHORT,KANUNGU CONTROL- YEAR FOUR [MITOOMA]
# KAGADI

```{r}
Mitooma_Year_4_Control_20 <- Mapping_Rukunjiri_2020_Year3_to_kanungu_Yr4_Controls %>%
  filter(District == "Mitooma", Cohort == "2020", Year == "Year 4", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Mitooma_2020 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Kagadi_2020_Year4_to_Mitooma_Yr4_Controls <- Mapping_Mitooma_2020_Year4_to_Mitooma_Year4_Control %>%
  left_join(Mitooma_Year_4_Control_20, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2020 &
...
Kanungu_Year_5_Control_18 <- Kanungu_2018_filtered_Year_3 %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 5", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Kanungu_2018 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kanungu_2018_Year5_to_Kanungu_Year5_Control <- Mapping_Kagadi_2020_Year4_to_Mitooma_Yr4_Controls %>%
  left_join(Kanungu_Year_5_Control_18, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2018 &
...
2019 COHORT,KANUNGU CONTROL- YEAR FIVE [KANUNGU]
# RUKUNJIRI

```{r}
Rukunjiri_Year_5_Control_18 <- Kanungu_2018_filtered_Year_3 %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 6", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Rukunjiri_2019 = Control) %>% view()

# Fill NAs in Control for Kisoro & Rukunjiri, Cohort 2019, Year "Base"
Mapping_Rukunjiri_2019_Year5_to_kanungu_Yr6_Controls <- Mapping_Kanungu_2018_Year5_to_Kanungu_Year5_Control %>%
  left_join(Rukunjiri_Year_5_Control_18, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2019 &
...
# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kanungu_2019_Year5_to_Kanungu_Year6_Control <- Mapping_Rukunjiri_2019_Year5_to_kanungu_Yr6_Controls %>%
  left_join(Kanungu_Year_5_Control_19, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
Kanungu_Year_4_Control <- Rubanda_2021_filtered_Year_3 %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 4", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Kanungu_Control_2020 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kanungu_2018_Year4_to_Kanungu_Year3_Control <- Rubanda_2021_filtered_Year_3 %>%
  left_join(Kanungu_Year_4_Control, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2018 &
      Year == "Year 4" &
      District %in% c("Kanungu"),
      Kanungu_Control_2020,
      Control
    )
  ) %>%
  select(-Kanungu_Control_2020) %>% view()

# Filter out Kanungu 2018 Year 3
Kanungu_2018_filtered_Year_4 <- Mapping_Kanungu_2018_Year4_to_Kanungu_Year3_Control %>%
  filter(!(District == "Kanungu" & Cohort == "2018" & Year == "Year 4" & is.na(RTV))) %>%
  view()
```
...
Mitooma_Year_4_Control <- Kanungu_2018_filtered_Year_4 %>%
  filter(District == "Mitooma", Cohort == "2020", Year == "Year 4", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Control_Mitooma_2020 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Mitooma_2020_Year4_to_Mitooma_Year4_Control <- Kanungu_2018_filtered_Year_4 %>%
  left_join(Mitooma_Year_4_Control, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
...
# Filter out Kanungu 2018 Year 3
Mitooma_2020_filtered_Year_4 <- Mapping_Mitooma_2020_Year4_to_Mitooma_Year4_Control %>%
  filter(!(District == "Mitooma" & Cohort == "2020" & Year == "Year 4" & is.na(RTV))) %>%
  view()
...
Kanungu_Year_5_Control <- Mitooma_2020_filtered_Year_4 %>%
  filter(District == "Kanungu", Cohort == "2018", Year == "Year 5", !is.na(Control)) %>%
  select(`Income Streams`, Control) %>%
  rename(Kanungu_Control_2020 = Control) %>% view()

# Fill NAs in Control for Kyenjojo, Cohort 2022, Year "Base"
Mapping_Kanungu_2018_Year4_to_Kanungu_Year3_Control <- Mitooma_2020_filtered_Year_4 %>%
  left_join(Kanungu_Year_5_Control, by = "Income Streams") %>%
  mutate(
    Control = ifelse(
      is.na(Control) &
      Cohort == 2018 &
...
# Filter out Kanungu 2018 Year 3
Kanungu_2018_filtered_Year_5 <- Mapping_Kanungu_2018_Year4_to_Kanungu_Year3_Control %>%
  filter(!(District == "Kanungu" & Cohort == "2018" & Year == "Year 5" & is.na(RTV))) %>%
  view()
